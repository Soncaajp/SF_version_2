...
class AffectnetIter(mx.io.DataIter):
	def __init__(self, data_json_path, batch_size, train, img_size, detector,multiply_basic_ratio=1, provide_thresholds=False):
		...
		if train == True:
			data = pd.read_csv(data_json_path)
		else:
			data = pd.read_csv(data_json_path)
		self.label2img = data
		...
		if self.provide_thresholds:
			...
			self.provide_label = [mx.io.DataDesc(name='softmax_label', shape=(batch_size,7))]
	...
	def next(self):
		...
		starter_path = '/media/nlab/data/AffectNet_Aligned/'
		for index, row in test.iterrows():
			if row['expression'] > 7 or row['subDirectory_filePath'].split('/')[1] not in os.listdir(starter_path+row['subDirectory_filePath'].split('/')[0]):
				count_bad += 1
		...
		for index, row in batch.iterrows():
			if row['expression'] > 7 or row['subDirectory_filePath'].split('/')[1] not in os.listdir(starter_path+row['subDirectory_filePath'].split('/')[0]):
				...
				# print(starter_path + row['subDirectory_filePath'])
				img = self.get_image([starter_path + row['subDirectory_filePath']])
				self.objects_processed += 1
				xb[i] = img
				l = []
				for i in range(1, 8):
					if i == int(row['expression']):
						l.append(1)
					else:
						l.append(0)
				yb[i] = l
