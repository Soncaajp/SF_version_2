...
def run_Affectnet_training():
    config = {'batch_size': 64,
              ...
              'save_model_prefix': '/media/nlab/data/AvgPool/resnext50-valence-avg',
              # 'emotions_list': ['Neutral', 'Happy', 'Sad', 'Surprise', 'Fear', 'Anger', 'Disgust', 'Contempt'],
              # 'multiply_basic_ratio': 4
              }
    train_iter = AffectnetIter(data_json_path='/media/nlab/data/AffectNet_Aligned/',
    ...
    # val_iter = AffectnetIter(data_json_path='/media/nlab/data/AffectNet_Aligned/',
    #                           batch_size=config['val_batch_size'], train=False, img_size=config['img_size'], detector = detector)
